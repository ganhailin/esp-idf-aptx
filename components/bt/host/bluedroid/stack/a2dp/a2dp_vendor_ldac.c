...
static const tA2DP_LDAC_CIE a2dp_ldac_source_caps = {
    ...
    (A2DP_LDAC_SAMPLERATE_44100 | A2DP_LDAC_SAMPLERATE_48000), /* sampleRate */
 ...
 */
tA2DP_STATUS A2DP_BuildInfoLDAC(uint8_t media_type,const tA2DP_LDAC_CIE* p_ie,uint8_t* p_result)
{
...
#define __DEBUG__LINE__OUTPUT APPL_TRACE_WARNING("%s:%d,%s",__FILE__,__LINE__,__func__)
tA2D_STATUS A2DP_ParseInfoLDAC(tA2DP_LDAC_CIE* p_ie,const uint8_t* p_codec_info,bool is_capability,char *caller_s)
{
  uint8_t losc;
  uint8_t media_type;
  ...
  // add by nishi
  APPL_TRACE_WARNING("a2d_vender_aptx.c::A2DP_ParseInfoLDAC() : #1 ,caller_s=%s",caller_s);

    APPL_TRACE_WARNING("a2d_vender_ldac.c::0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x",
                       p_codec_info[0],p_codec_info[1],p_codec_info[2],p_codec_info[3],p_codec_info[4],p_codec_info[5],
                       p_codec_info[6],p_codec_info[7],p_codec_info[8],p_codec_info[9],p_codec_info[10]);
  ...
  if (p_ie->vendorId != A2DP_LDAC_VENDOR_ID ||
      p_ie->codecId != A2DP_LDAC_CODEC_ID_BLUETOOTH) {
    return A2DP_WRONG_CODEC;
  }
    __DEBUG__LINE__OUTPUT
  ...
  if (is_capability) {
    ...
    if (A2DP_BitsSet(p_ie->sampleRate) == A2D_SET_ZERO_BIT)
      return A2DP_BAD_SAMP_FREQ;
    if (A2DP_BitsSet(p_ie->channelMode) == A2D_SET_ZERO_BIT)
      return A2DP_BAD_CH_MODE;

    return A2DP_SUCCESS;
  }
    __DEBUG__LINE__OUTPUT
  if (A2DP_BitsSet(p_ie->sampleRate) != A2D_SET_ONE_BIT)
    return A2DP_BAD_SAMP_FREQ;
  if (A2DP_BitsSet(p_ie->channelMode) != A2D_SET_ONE_BIT)
    return A2DP_BAD_CH_MODE;
    __DEBUG__LINE__OUTPUT
  return A2DP_SUCCESS;
}
...
**
*******************************************************************************/
UINT8 bta_av_ldac_cfg_in_cap(UINT8 *p_cfg, tA2DP_LDAC_CIE *p_cap)
{
    UINT8           status = 0;
